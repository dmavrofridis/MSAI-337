...
def tagging(dialogue):
    ...
    for i in range(len(dialogue_tokens)):
        ...
        dialogue_tokens[i] = re.sub(r'[0-9]+[\s]*-[\s]*[0-9]+', '<address>', dialogue_tokens[i])
        dialogue_tokens[i] = re.sub(r'[0-9]+[\s]*[0-9]+', '<phone-number>', dialogue_tokens[i])
        dialogue_tokens[i] = re.sub(r'[0-9]+', '<integer>', dialogue_tokens[i])
    return listToString(dialogue_tokens, " ")
...
def main():
    ...
    gen_mode = 2
    gen_labels = ['logits', 'greedy', 'beam', 'top-p', 'custom']
    tuned_model = 1

    if tuned_model == 0:
        generate_dir_if_not_exists(output_data_path + '/gpt2')
        tuned = output_data_path + '/gpt2'
